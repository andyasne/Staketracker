...
namespace Staketracker.Core.Services
{
    public class ApiManager : IApiManager
    {
        ...
        public async Task<HttpResponseMessage> GetProjectTeam(jsonTextObj jsonTextObj, string sessionId)
        {
            var cts = new CancellationTokenSource();
            var task = RemoteRequestAsync<HttpResponseMessage>(staketrackerApi.GetApi(Priority.UserInitiated).GetProjectTeam(jsonTextObj, sessionId, cts.Token));
            return await AddToRunningTasks(cts, task);

        }
        public async Task<HttpResponseMessage> GetIssues(jsonTextObj jsonTextObj, string sessionId)
        {
            var cts = new CancellationTokenSource();
            var task = RemoteRequestAsync<HttpResponseMessage>(staketrackerApi.GetApi(Priority.UserInitiated).GetIssues(jsonTextObj, sessionId, cts.Token));
            return await AddToRunningTasks(cts, task);
        ...
        public async Task<HttpResponseMessage> GetProjectTeamMemberDetails(jsonTextObj jsonTextObj, string sessionId)
        {
            var cts = new CancellationTokenSource();
            var task = RemoteRequestAsync<HttpResponseMessage>(staketrackerApi.GetApi(Priority.UserInitiated).GetProjectTeamMemberDetails(jsonTextObj, sessionId, cts.Token));
            return await AddToRunningTasks(cts, task);

        }
        public async Task<HttpResponseMessage> DelRec(jsonTextObj jsonTextObj, string sessionId)
        {
            var cts = new CancellationTokenSource();
            var task = RemoteRequestAsync<HttpResponseMessage>(staketrackerApi.GetApi(Priority.UserInitiated).DelRec(jsonTextObj, sessionId, cts.Token));
            return await AddToRunningTasks(cts, task);
        ...
        public async Task<HttpResponseMessage> GetProjectList(jsonTextObj jsonTextObj, string sessionId)
        {
            var cts = new CancellationTokenSource();
            var task = RemoteRequestAsync<HttpResponseMessage>(staketrackerApi.GetApi(Priority.UserInitiated).GetProjectList(jsonTextObj, sessionId, cts.Token));
            return await AddToRunningTasks(cts, task);
